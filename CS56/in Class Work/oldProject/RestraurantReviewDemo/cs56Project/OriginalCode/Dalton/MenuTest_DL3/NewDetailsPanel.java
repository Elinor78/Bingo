...
public class NewDetailsPanel extends JPanel {
    ...
    private JTextArea taComments = new JTextArea(5, 30);
    ...
    public NewDetailsPanel()
    {
        ...
        // addComponent(state, 3,0,2,1);
        // addComponent(tfState, 3 ,2,1,1);
        // addComponent(zip, 4,0,2,1);
        // addComponent(tfZip, 4,2,2,1);
        // addComponent(phone, 5,0,2,1);
        // addComponent(tfPhone, 5,2,2,1);

        addComponent(overall, 6, 0, 2, 1);
        for (int i = 0; i < rgOverall.getButtonCount(); i++) {
            addComponent(overallRadioButtons[i], 6, i + 2, 1, 1);
        }
        addComponent(price, 7, 0, 2, 1);
        for (int i = 0; i < rgPrice.getButtonCount(); i++) {
            addComponent(priceRadioButtons[i], 7, i + 2, 1, 1);
        }
        addComponent(kidsFriendly, 8, 0, 2, 1);
        for (int i = 0; i < rgKids.getButtonCount(); i++) {
            addComponent(kidsRadioButtons[i], 8, i + 2, 1, 1);
        }
        addComponent(comments, 9, 0, 2, 1);
        addComponent(taComments, 9, 2, 8, 1);
