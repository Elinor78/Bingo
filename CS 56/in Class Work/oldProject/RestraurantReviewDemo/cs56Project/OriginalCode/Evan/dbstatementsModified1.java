...
public class dbstatementsModified1 extends AbstractTableModel implements Subject, Observer, NotifyConstants
{
    private ArrayList<Observer> observers = new ArrayList<Observer> ();
    
	static final String DRIVER = "com.mysql.jdbc.Driver"; // This needs to be changed
	static final String DBURL = "jdbc:mysql://localhost:3306/restaurants"; // change this 
	...
	private final String insQuery = "INSERT INTO rests (name,address,cuisine,foodRat,serviceRat,ambianceRat,overallRat) VALUES ('Default','Default','Default',1,1,1,1)";
	private final String delQuery = "DELETE FROM rests WHERE ";
	 ...
	 * values[] should always be a four element array in the following order: name, cuisine, city, overallRat
	 */
	public void find( String values[] ) throws SQLException, IllegalStateException
	{
		if( !connectedToDB )
		...
		{
			if(addAnd)
				query += " AND cuisine=" + values[1];
			else
				query += "cuisine=" + values[1];
		}
		if(values[2] != "" && values[2] != null)
		{
			if(addAnd)
				query += " AND city=" + values[2];
			else
				query += "city=" + values[2];
		}
		...
		{
			if(addAnd)
				query += " AND overallRat=" + values[3];
			else
				query += "overallRat=" + values[3];
		}
	...
	public void giveMeAll() throws /*SQLException,*/ IllegalStateException
	{
		...
		try
		{
            ...
            resultSet.last();
			numRows = resultSet.getRow();
            resultSet.first();  // point result set back to first row
			System.out.println("Finished running query"); //debug
		}
		catch( SQLException sql )
		{
    ...
    public void update( Subject s, Object arg )
    {
        if (s instanceof TaskPane && arg instanceof Integer) {
            ...
            if (notify == PANEL_UPDATE) {
                // use getters to update dbstatements
                fireTableStructureChanged();
                System.out.println( "dbstatements updated by TaskPane." );
            }
        }
        /*
        if (s instanceof ReviewTablePane && arg instanceof Integer) {
            ReviewTablePane rtp = (ReviewTablePane) s;
            int notify = (Integer) arg;
            
            if (notify == LIST_UPDATE) {
                try {
                    //resultSet.absolute(rtp.getSelectedRow() + 1);
                    System.out.println( this.getName() + " is at row " + resultSet.getRow() );
                    System.out.println( "resultSet points to row " + resultSet.getRow() );
                    System.out.println( this.getName() + " is at row " + resultSet.getRow() );
