...
public class RestDB extends AbstractTableModel
{
	static final String DRIVER = "com.mysql.jdbc.Driver"; // This needs to be changed
	static final String DBURL = "jdbc:mysql://localhost/restaurants?user=user&password=password"; //This needs to be changed
	...
	public void find( HashMap map ) throws SQLException, IllegalStateException
	{
		...
		{
			query += "name=" + (String)map.get("Name");
			addAnd = true;
		}
		if (map.containsKey("City"))
		{
			if(addAnd)
				query += " AND city=" + (String)map.get("City");
			else
			{
				query += "city=" + (String)map.get("City");
				addAnd = true;
			}
		}
		...
		{
			if(addAnd)
				query += " AND cuisine=" + (String)map.get("Food");
			else
			{
				query += "cuisine=" + (String)map.get("Food");
				addAnd = true;
			}
		}
		...
		{
			if(addAnd)
				query += " AND overallRat=" + (String)map.get("Overall");
			else
				query += "overallRat=" + (String)map.get("Overall");
		}
